...
export const applyGravity = (bricks: Brick[]) => {
  bricks.forEach((brick, index) => {
    ...
    while (brick.from[2] > 1 && localI >= 0) {
      const previousBricks = bricks.filter(elem => elem.to[2] === (brick.from[2] - 1)).filter(elem => isOverlap(elem, brick))
...
const isOverlap = (obj1: Brick, obj2: Brick): boolean => {
  const overlapX = (obj1.from[0] <= obj2.to[0] && obj1.to[0] >= obj2.from[0]) ||
        (obj2.from[0] <= obj1.to[0] && obj2.to[0] >= obj1.from[0])

  const overlapY = (obj1.from[1] <= obj2.to[1] && obj1.to[1] >= obj2.from[1]) ||
...
export const calculateDisintegratableBricks = (bricks: Brick[]): number => {
  ...
  bricks.forEach((brick, index) => {
    const higherBricks = bricks.filter(elem => elem.from[2] === (brick.to[2] + 1)).filter(elem => isOverlap(elem, brick))
    ...
    if (higherBricks.length === 0) {
      ...
      const noneFallsDone = higherBricks.every(element => {
        const previousBricks = bricksWithoutCurrentBrick.filter(elem => elem.to[2] === (element.from[2] - 1)).filter(elem => isOverlap(elem, element))
...
export const countFallingBlocksWhileApplyingGravity = (bricks: Brick[]): number => {
  ...
  myBricks.forEach(brick => {
    while (brick.from[2] > 1) {
      const previousBricks = myBricks.filter(elem => elem.to[2] === (brick.from[2] - 1)).filter(elem => isOverlap(elem, brick))
      if (previousBricks.length === 0) {
        brick.from[2] -= 1
        brick.to[2] -= 1
        brick.fallen = true
