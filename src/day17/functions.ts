...
export const dijkstraPartOne = (graph: number[][]): Array<Array<{
  left: number[]
  right: number[]
  top: number[]
  bottom: number[]
  ...
  distances[0][0].left[0] = 0
  distances[0][0].right[0] = 0
  distances[0][0].top[0] = 0
  distances[0][0].bottom[0] = 0
...
export const dijkstraPartTwo = (graph: number[][]): Array<Array<{
  left: number[]
  right: number[]
  top: number[]
  bottom: number[]
  ...
  const queue: Node[] = []
  const distances = graph.map(row => row.map(_ => ({
    left: [Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE],
    right: [Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE],
    top: [Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE],
    bottom: [Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE, Number.MAX_VALUE]
  })))
  distances[0][0].left[0] = 0
  distances[0][0].right[0] = 0
  distances[0][0].top[0] = 0
  ...
  while (queue.length > 0) {
    ...
    directions.forEach(direction => {
      ...
      if (neighborExists(width, height, neighborY, neighborX)) {
        ...
        if (distances[neighborY][neighborX][newDirection][newAmountStraight - 1] > node.distance + graph[neighborY][neighborX] && newAmountStraight < 11) {
          if (!(direction.x === 1 && node.direction === 'left') && !(direction.x === -1 && node.direction === 'right') && !(direction.y === 1 && node.direction === 'top') && !(direction.y === -1 && node.direction === 'bottom')) {
            if ((newDirection === node.direction || node.amountStraight > 3)) {
              if (neighborX === graph[0].length - 1 && neighborY === graph.length - 1 && newAmountStraight > 3) {
                queue.push({
                  x: neighborX,
                  y: neighborY,
                  distance: node.distance + graph[neighborY][neighborX],
              ...
              } else if (!(neighborX === graph[0].length - 1 && neighborY === graph.length - 1)) {
                queue.push({
                  x: neighborX,
                  y: neighborY,
                  distance: node.distance + graph[neighborY][neighborX],
